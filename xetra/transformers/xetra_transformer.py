...
class XetraSourceConfig(NamedTuple):
    ...
    src_col_insin: column name for isin in source
...
class XetraTargetConfig(NamedTuple):
    ...
    trg_col_insin: column name for isin in target
    trg_col_op_price: column name for opening price in target
    trg_col_clos_price: column name for starting price in target
    trg_col_min_price: column name for minimum price in target
    trg_col_max_price: column name for maximum price in target
    trg_col_daily_traded_vol: column name for daily traded volume in target
    trg_col_ch_prev_clos: cloumn name for change to previous day<s sling price in target
...
class XetraETL():
    """
    Reads the Xetra data, tranforms and writes the transformed to target
    """
